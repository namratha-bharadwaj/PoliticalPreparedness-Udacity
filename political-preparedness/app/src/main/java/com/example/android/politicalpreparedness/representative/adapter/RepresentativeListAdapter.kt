...
class RepresentativeViewHolder(val binding: ItemRepresentativeBinding): RecyclerView.ViewHolder(binding.root) {
    ...
    fun bind(item: Representative) {
        ...
        item.official.channels?.let { socialMediaChannels ->
            showSocialLinks(socialMediaChannels) }

        //Complete: Show www link ** Hint: Use provided helper methods
        item.official.urls?.let { webUrls ->
            showWWWLinks(webUrls)
        }
    ...
    private fun showWWWLinks(urls: List<String>) {
        enableLink(binding.wwwImageView, urls.first())
    }
